...
def function_2p(n_param):
    ...
    ax.grid(False)
    #ax.set_title(r"$\mathrm{Average\ energy \calculation}~~\langle E_{%d}(α, β) \rangle = \frac{\langle \Psi_{%d}(x) | \hat{H} | \Psi_{%d}(x) \rangle}{\langle \Psi_{%d}(x) | \Psi_{%d}(x) \rangle}$" % (n, n, n, n, n), fontsize=16, family="serif")
