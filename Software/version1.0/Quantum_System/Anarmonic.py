...
def Anarmonic():   
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('V(x, y)')
    ax.set_title(r"$\left[ \frac{\hat{p}^2}{2m} + \frac{1}{2} m \omega^2 (\hat{x}^2 + \hat{y}^2) - 0.01 (\hat{x}^4 + \hat{y}^4) \right] |\psi\rangle = E |\psi\rangle$", fontsize=16)
...
def minimizacion(a_values, b_values, l_r, e_p, p_1, p_2):
    ...
    print(f"E({round(theta_a, 2)}, {round(theta_b, 2)}) = {round(E_min, 2)}")
    ...
    def update(frame):
        ...
        title.set_text(fr"$\hat{{H}} | \Psi_{{{n}}} \rangle = {round(E_z[frame - 1], 2)} |\Psi_{{{n}}} \rangle, \quad J = \frac{{1}}{{N}} \sum_{{i=1}}^{{N}} \left| \hat{{H}} |\Psi_{{{n}}} \rangle - E_{{{n}}} |\Psi_{{{n}}} \rangle \right|^2 = {np.round(np.mean(np.abs(-0.5 * d2_f + V * f  -  round(E_z[frame - 1], 2) * f)), 3)}$")
        return line, title
